...
function resolveContent(resource) {
	// In case resource.content() throws, put resource inside a promise first
	return when(resource).then(function(resource) {
		return keys.all({ resource: resource, content: resource.content() });
	}).else();
}
